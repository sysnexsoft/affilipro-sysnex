                ...
                <form action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data" id="blogForm">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-8">
                            ...
                            <select name="product_ids[]" class="form-control select2-products" multiple="multiple">
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}">{{ $product->title }} (${{ $product->sale_price ?? $product->regular_price }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="url" name="affiliate_url" class="form-control" placeholder="https://amazon.com/..." value="{{ old('affiliate_url') }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-bold">Affiliate Source/Network</label>
                            <input type="text" name="affiliate_source" class="form-control" placeholder="e.g. Amazon, ClickBank" value="{{ old('affiliate_source') }}">
                        </div>
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.summernote').summernote({ height: 350, placeholder: 'Write content here. Type __product__ anywhere to place product box...' });
